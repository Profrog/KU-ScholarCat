...
- `KSKILL_PROXY_TRUST_PROXY_HOPS` — Fastify가 신뢰할 reverse-proxy hop 수, 애플리케이션 기본값 `0`. gpu01 production 배포에서만 배포 스크립트가 누락된 값을 `1`로 추가한다. staging·로컬·다른 호스트와 직접 노출되는 서버에는 이 production 기본값을 적용하지 않는다. hops가 1 이상이면 rate limiter는 `CF-Connecting-IP`를 `X-Forwarded-For`보다 우선한다.
