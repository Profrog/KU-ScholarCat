...
이 스킬은 세무사법상 세무대리(기장대행·세무신고 대행 등)를 수행하지 않는다는 전제로만 운영한다.

1. **참고용 초안 한정이다.** 장부·재무제표·세무사 전달 CSV는 사용자가 세무사와 함께 검토하기 위한 참고 자료로 한정하고, 신고 대행·기장대행·절세 자문 기능은 제공하지 않는다.
2. **본인 장부 전용이다.** 사용자가 자신의 사업자 데이터를 정리하는 용도로 한정하고, 제3자의 장부를 대가를 받고 대행하는 형태로 운영하지 않는다.
3. **신고 전 전문가 확인을 강제한다.** 종합소득세·법인세 신고 목적이면 산출물을 최종본으로 단정하지 말고 세무사 검토(외감 대상이면 공인회계사 감사)를 워크플로에 고정한다.
4. **데이터는 로컬 처리가 기본이다.** CODEF 자격증명은 BYOK 로컬 저장소만 쓰고, 거래내역에 포함된 거래자·직원 개인정보는 마스킹(Level 2) 후에만 LLM fallback에 전달한다. 외부 전송 경로를 사용자 승인 없이 확장하지 않는다.
