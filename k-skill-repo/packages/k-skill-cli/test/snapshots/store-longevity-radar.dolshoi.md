# store-longevity-radar — assembled instructions
...
- Execute bundled helpers only through `npx -y @nomadamas/k-skill@0 exec store-longevity-radar scripts/<file> -- <args>`; do not assume a repository-relative or installed-skill-relative path.
- Resolve an asset path with `npx -y @nomadamas/k-skill@0 path store-longevity-radar <relative-path>` only when another tool explicitly requires a filesystem path.
